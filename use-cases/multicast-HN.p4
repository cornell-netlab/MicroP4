/*
 * Data-plane for multicast. This program illustrates parallel composition of
 * multicast-MNF-N.p4 and multicast-MNF-H.p4.
 *
 *
 */
...
control ingress(inout Parsed_headers ph, inout ingress_metadata_t  meta,
  ...
  action mnf_h_set_multicast_group(bit<16> multicast_group) {
    standard_metadata.mcast_grp = multicast_group;
  }

  action mnf_h_decap_cpu_header() {
    ph.cpu_header.setInvalid();
  }

  action mnf_h_encap_cpu_header(bit<9> cpu_port) {
  ...
  table mnf_h_igmp_table {
    ...
    actions = {
      mnf_h_decap_cpu_header;
      // set the interfaces for which the switch is querier
      mnf_h_set_multicast_group;
      mnf_h_encap_cpu_header;
      NoAction;
    }
    default_action = NoAction();
  }

  table mnf_h_data_multicast{
    key = {
  ...
  action mnf_n_set_multicast_group(bit<16> multicast_group) {
    standard_metadata.mcast_grp = multicast_group;
  }

  table mnf_n_replicate_upstream {
    key = {
      // src address for source-specific multicast
      ph.ipv4.srcAddr: exact;
      // group address
  ...
  apply {
    if (ph.igmp.isValid())
      mnf_h_igmp_table.apply();
    if (ph.ipv4.isValid() && ph.udp.isValid()) {
      mnf_h_data_multicast.apply();
      mnf_n_replicate_upstream.apply();
    }
  }
}

...
const cstring CreateTofinoArchBlock::egIMArgName = "ingress_intrinsic_metadata_from_parser_t";
const cstring CreateTofinoArchBlock::igIMdpArgName = "ingress_intrinsic_metadata_for_deparser_t";
...
const IR::Node* CreateTofinoArchBlock::createTofinoIngressParser() {
    ...
    cstring parseOneByteStateName = "parse_one_byte";
    ...
    {   // creating start state
        ...
        auto pe = new IR::PathExpression(parseTwoBytesStateName);
        IR::IndexedVector<IR::StatOrDecl> components;
        components.push_back(setCounterMCS);

        auto pe = new IR::PathExpression(parseTwoBytesStateName);

        auto ps = new IR::ParserState(IR::ParserState::start, components, pe);
        states.push_back(ps);
    }
     
    {
        ...
        auto mce = new IR::MethodCallExpression(isZeroM);
        ev.push_back(cond);
        ...
        auto scf = new IR::SelectCase(new IR::BoolLiteral(false), 
            new IR::PathExpression(parseTwoBytesStateName));
        auto sct = new IR::SelectCase(new IR::BoolLiteral(true), 
            new IR::PathExpression(parseOneByteStateName));
        selectCases->push_back(scf);
        selectCases->push_back(sct);
        auto se = new IR::SelectExpression(ls, *selectCases);
        auto ps = new IR::ParserState(parseTwoBytesStateName, components, se);
        states.push_back(ps);
    }   
...
IR::P4Control* CreateTofinoArchBlock::createTofinoPipelineControl(cstring name, 

...
cpackage IPv6EXT : implements Unicast<l3v6_hdr_t, l3_meta_t, empty_t, bit<16>, empty_t> {
  ...
  control micro_control(pkt p, im_t im, inout l3v6_hdr_t hdr, inout l3_meta_t m,
	...
	action process(bit<16> nh){
	  ...
	  hdr.ipv6.srcAddr = srv6io.srcAddr;
	  hdr.ipv6.totalLen = srv6io.totalLen;
      hdr.ipv6.hoplimit = srv6io.hoplimit - 1;
      nexthop = nh;
    }
    action default_act() {
      nexthop = 0; 
    ...
    apply {
    	...
    	srv6io.nexthdr = hdr.ipv6.nexthdr;
    	if (hdr.ipv6.nexthdr == 43)
    		srv6_i.apply(p, im, e, nexthop, srv6io);
      	ipv6_lpm_tbl.apply(); 
    }
  }

...
struct natting_hdr_t {
  ethernet_h ethernet;
  ipv4_h ipv4;
  tcp_h tcp;
}

parser ParserImpl (packet_in pin, out natting_hdr_t parsed_hdr, 
                inout natting_meta_t meta, 
...
control ingress(inout natting_hdr_t parsed_hdr, inout natting_meta_t meta,
                 inout standard_metadata_t standard_metadata) {	
                         action drop_action() {
            meta.drop_flag = 1;
        }
...
control egress(inout natting_hdr_t parsed_hdr, inout natting_meta_t meta,
                 inout standard_metadata_t standard_metadata) {	
...
control DeparserImpl(packet_out packet, in  natting_hdr_t hdr) {
    apply {
...
control verifyChecksum(inout  natting_hdr_t hdr, inout natting_meta_t meta) {
    apply {
    }
}

control computeChecksum(inout  natting_hdr_t hdr, inout natting_meta_t meta) {
    apply {
    }
}

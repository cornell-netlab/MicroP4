...
control ingress(inout routerv46_hdr_t parsed_hdr, inout routerv46_meta_t meta,
    ...
    table ipv6_lpm_tbl {
     ...
     action v4_v6(){
     	parsed_hdr.ethernet.etherType = 0x86DD; 
	    parsed_hdr.ipv4.setInvalid();
	    parsed_hdr.ipv6.setValid();
	    parsed_hdr.ipv6.version = 6;
 		parsed_hdr.ipv6.class = parsed_hdr.ipv4.diffserv;
  		parsed_hdr.ipv6.label = 0;
 		parsed_hdr.ipv6.totalLen = parsed_hdr.ipv4.totalLen - (bit<16>)(parsed_hdr.ipv4.ihl);
  		parsed_hdr.ipv6.nexthdr = parsed_hdr.ipv4.protocol; 
  		parsed_hdr.ipv6.hoplimit = parsed_hdr.ipv4.ttl;
  		parsed_hdr.ipv6.srcAddr = (bit<128>)parsed_hdr.ipv4.srcAddr;
  		parsed_hdr.ipv6.dstAddr = (bit<128>)parsed_hdr.ipv4.dstAddr; 
    }

	action v6_v4(){
     	parsed_hdr.ethernet.etherType = 0x0800; 
		parsed_hdr.ipv6.setInvalid();
	    parsed_hdr.ipv4.setValid();
	  	parsed_hdr.ipv4.version = 4;
	  	parsed_hdr.ipv4.ihl = 5;
  		parsed_hdr.ipv4.diffserv = parsed_hdr.ipv6.class;
  		parsed_hdr.ipv4.totalLen = parsed_hdr.ipv6.totalLen +  (bit<16>)(parsed_hdr.ipv4.ihl);
  		parsed_hdr.ipv4.identification = 0;
		parsed_hdr.ipv4.flags = 0;
  		parsed_hdr.ipv4.fragOffset = 0;
  		parsed_hdr.ipv4.ttl = parsed_hdr.ipv6.hoplimit;
  		parsed_hdr.ipv4.protocol = parsed_hdr.ipv6.nexthdr;
  		parsed_hdr.ipv4.hdrChecksum = 0;
  		parsed_hdr.ipv4.srcAddr = (bit<32>)(parsed_hdr.ipv6.srcAddr << 96);
  		parsed_hdr.ipv4.dstAddr = (bit<32>)(parsed_hdr.ipv6.dstAddr << 96); 
	}

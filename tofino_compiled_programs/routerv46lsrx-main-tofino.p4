...
control MplsLSR_micro_parser(inout msa_packet_struct_t p, out mpls_hdr_t hdr, out MplsLSR_parser_meta_t parser_meta) {
    ...
    table parser_tbl {
        ...
        const entries = {
                        (16w112, 8w0) : i_112_parse_mpls1_0();
...
control MplsLSR_micro_control(inout ingress_intrinsic_metadata_for_deparser_t ig_intr_md_for_dprsr, inout mpls_hdr_t hdr, out bit<16> nextHop, inout bit<16> eth_type, in MplsLSR_parser_meta_t parser_meta, inout MplsLSR_hdr_vop_t hdr_vop) {
    ...
    @name("MplsLSR.micro_control.decap") action decap(bit<16> t) {
    ...
    @name("MplsLSR.micro_control.mpls_tbl") table mpls_tbl_0 {
        key = {
            parser_meta.mpls1_v || hdr_vop.mpls1_sv: exact @name("hdr.mpls1.$valid$") ;
...
control MplsLSR(inout msa_packet_struct_t msa_packet_struct_t_var, inout ingress_intrinsic_metadata_for_deparser_t ig_intr_md_for_dprsr, out bit<16> out_param, inout bit<16> inout_param) {
    ...
    apply {
        MplsLSR_hdr_vop_t_var.mpls1_sv = false;
        MplsLSR_hdr_vop_t_var.mpls1_siv = false;
        MplsLSR_hdr_vop_t_var.mpls0_sv = false;
        MplsLSR_hdr_vop_t_var.mpls0_siv = false;
        MplsLSR_micro_parser_inst.apply(msa_packet_struct_t_var, mpls_hdr_t_var, MplsLSR_parser_meta_t_var);
        MplsLSR_micro_control_inst.apply(ig_intr_md_for_dprsr, mpls_hdr_t_var, out_param, inout_param, MplsLSR_parser_meta_t_var, MplsLSR_hdr_vop_t_var);
        ...
        if (hdr.eth.ethType == 16w0x800) 
            ...
            if (hdr.eth.ethType == 16w0x86dd) 
                ...
                if (hdr.eth.ethType == 16w0x8847) 
                    mpls_i_0.apply(msa_packet_struct_t_var, ig_intr_md_for_dprsr, nh_0, hdr.eth.ethType);
        forward_tbl_0.apply();
    }
}

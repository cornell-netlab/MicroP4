...
To support data plane programming of variety of target devices, the ecosystem expects manufacturers to provide a framework to program the target devices.
The framework includes an architecture definition in P4 and compiler for the target.
The architecture description (e.g., v1model.p4 \cite{v1model.p4} for simple\_switch) of a target specifies data plane pipeline comprising a set of programmable and fixed-function blocks, flow of user-defined and target-specific (called intrinsic) metadata in the pipeline and semantics of target-specific actions and externs.
...
To program the packet processing pipeline of a device, P4 programmers provide implementation of the declared programmable blocks in its architecture.
This makes P4 programs tightly coupled with the architecture of the target devices.
...
Heterogeneity in programmable blocks within an architecture makes reuse of code libraries for the same device extremely difficult.
...
Packet processing logic in P4 programs are partitioned across programmable blocks with heterogeneous abstract machines (e.g., parsers, deparsers, control) and fixed-function blocks (Packet Buffer and Replication Engine).
...
Consider the example in Figure \ref{fig:l2.p4}, parser \texttt{P} of \texttt{l2} can not be executed at the end of deparser control block \texttt{D} of \texttt{ipv4} \ref{fig:ipv4.p4}
...
In turn, increasing complexity in packet-processing architecture model of the architecture.
...
Finally, the existing architecture models expose target-specific constraints to programmers.
...
It also generates architecture-independent control-plane APIs, P4Runtime \cite{p4run-time} files.

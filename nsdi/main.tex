...
%% tables~\cite{bosshart2013forwarding}.} 
...
% (Composition)
\tightparbf{Challenge 3: Heterogeneous programming model.}%
%
...
\texttt{h} is defined. The obvious way to compose these programs would
...
\tightpar{Modular:} It should be possible to develop individual
...
\tightpar{Composable:} It should be easy to compose individual
...
too low level and too monolithic, as highlighted in
~\cref{sec:challenges}. Consequently, we propose to raise the level of
abstraction so that programs can be expressed in terms of a suitable
external interface, while encapsulating the internal features needed
to implement a given packet-processing function. Of course,
introducing yet another architecture to unify existing architectures
would add significant complexity to the ecosystem. Instead, we propose
a \emph{logical} architecture that acts as high-level description
against which programs can be written. We then map this logical
architecture down to the standard architectures supported by P4
targets. This architecture is logical in the sense that devices do not
explicitly implement it; rather, it captures the bare essence of
packet processing and exposes that to programmers including:
\begin{enumerate*}[label=(\roman*)]
  ...
  \item A generic way to use features such as externs
    without introducing tight coupling to targets, and
...
these to show novel ways of composing dataplane applications.
 \deleted{\ulang programs to build real applications.}
...
\ulang modules for different network layers.} 
To illustrate, we
...
% and compiles them to target-specific low-level microcode.

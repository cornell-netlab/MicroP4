...
sub-language whose abstract machine models imperative control flow. As
each abstract machine is defined independently, there is no common
interface that programmers can write their packet-processing modules
against. This lack of uniform abstract machine and interface prevents
packet-processing modules to be composed together as composition
requires modules to have compatible interfaces. Going back to the
earlier example, in \cref{fig:l2.p4,fig:ipv4.p4}, ideally we would
like to execute the \texttt{ipv4} module right before applying
...
programming model that prevents hinders code reuse, modularity and
composition:
\begin{enumerate}
  ...
  \item Architectures do not provide a clean abstraction that can
    decouple dataplane programs from targets. This results in
    dataplane programs being closely tied to a target, and it is
    difficult to develop programs in a clean and modular way.
...
architectures.
\tightpar{Modular:} Individual dataplane functionality can be
...
\ucomp's backend transforms all the control blocks by $(1)$ translating \uarch-specific constructs to target-specific ones and $(2)$ allocating the control blocks to ones of target pipeline model.
...
Finally, {copy\_from} method calls of \texttt{pkt} instances are replaced for v1model with \texttt{clone3} followed by \texttt{recirculate} or \texttt{resubmit} while setting appropriate thread-id user-defined metadata.
We can perform similar transformation for other architectures like PSA using appropriate functions exposed by them.
...
\ucomp is implemented with \textasciitilde 6K LoCs and as an extension of open source reference compiler p4c \cite{p4c}.
We discuss our implementation in reference with specification of P4-16.

\subsubsection{Comparison with P4}
\ucomp allows to define custom package types, similar to control and parser blocks, which is not feasible with current specification P4-16. 
Also, for abstracting implementation of package types, \uarch defines interfaces encapsulating a set of programmable blocks.
...
Our prototype \ucomp minimally extends P4 grammar and frontend to implement features required for code reuse and modularity.
\ulang do not violate semantics of any other programming constructs (externs, control and parser blocks) defined in P4-16's specification.


\subsubsection{Supporting Target Architectures}

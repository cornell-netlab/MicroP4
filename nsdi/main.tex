...
specifying the packet-processing behaviour of 
\hse{re-configurable} \deleted{programmable switching}
chips, such as Barefoot Tofino~\cite{tofino} and Cavium
Xpliant~\cite{xpliant}, which are based on \hse{a programmable 
parser} and a pipeline of match-action
...
programmable and fixed-function blocks, flow of \hse{packet}, user-defined and
target-specific (called \emph{intrinsic}) metadata in the pipeline, and
semantics of target-specific \hse{constructs (actions and externs)}. For 
example,
...
\hse{To program a target, one can use P4 to specify the behavior
of the programmable blocks exposed in the architecture of the target.
While, this approach allows supporting multiple targets which share the
same architecture, it also makes P4 programs dependent on
$(1)$ dataplane pipeline, $(2)$ flow of packet and metadata in
the pipeline and  $(3)$ target-specific constructs. These 
dependencies pose multiple challenges for architecture-agnostic reuse 
of independently developed P4 programs to build new ones}
\deleted{to reuse P4 programs
across targets with different and, even, the same architectures, as
explained next.}
...
% (Portability)
\paragraph{Challenge 1: P4 programs are specific to architecture.}
\deleted{Architectures differ in the way their dataplane pipeline is arranged.}
\hse{The structure of a P4 program is dependent on a 
dataplane pipeline model which is specified by the architecture of 
the program's target. Architectures differ in the set of 
programmable-blocks and their arrangement in pipeline.}
So, if the architecture changes, it is often impossible to \hse{reuse}
the P4 program \hse{with the pipeline model of} the new architecture 
without a complete rewrite. For example, in 
\cref{fig:arch-example}, \hse{to map the functionality implemented 
for PSA's ingress deparser block to any programmable block in 
v1model's pipeline, semantic understanding of the blocks are 
required. Therefore, there is no obvious and automatic way, but 
manual rewrite, to reuse the implementation of programmable blocks 
across different pipeline model.}
\deleted{there is no obvious and automatic way to map the 
...
% (... and modularity)
\hse{Moreover, to program pipeline of an architecture, P4 
programs have to rely heavily on the architecture-specific intrinsic 
metadata and constructs (e.g., resubmit, recirculate, clone, etc.) 
enabling special processing such as packet 
replication~\cite{simple_switch.md,psa}.
These dependence hurt portability of P4 programs across different 
architectures.}

\deleted{Moreover, current architectures expose certain 
target-specific information and constructs---such as intrinsic 
metadata, actions and \emph{extern} functions (e.g., resubmit,
...
architectures and targets which hurts portability.}
% \sout{also prohibits modular development.}

\paragraph{Challenge 2: High coupling among programmable blocks 
exposed by the same architecture.} \hse{
A P4 program specifies the behavior of programmable blocks 
exposed by the architecture of its target. An architecture
specifies a flow of packet, user-defined and intrinsic metadata 
passing through programmable blocks in its pipeline. This results in 
highly coupled block-specific P4 modules in every P4 program. For 
example, In \cref{fig:arch-example}, PSA's ingress parser initializes 
a set of declarations, packet headers and user-define metadata which 
are passed to and processed by ingress control. To reuse the 
ingress control module of a P4 program in another PSA-specific 
program, it is mandatory to expose all the declarations passed to 
the module. Because these declarations are initialized by the 
parser block, its implementation should be known or it must be reused 
along with the ingress module. The high coupling and global 
scope of data in pipeline makes extremely difficult to develop 
independent modules, thereby hindering modularity.}

% \hs{text explaining split of switch.p4 did not read well today.}

\deleted{For example, many P4 programs written for PSA
rely heavily on using intrinsic metadata supported by PSA, and such
metadata is used across a program in an unrestricted
manner~\cite{switch.p4};
it is difficult to split such a program into
clean modules which can be \hse{reused} developed independently.  
More fundamentally, it breaks the notion of abstraction needed to 
write 
...
% (Composition)
\paragraph{Challenge 3: Programmable blocks with heterogeneous 
abstract machines.}
\deleted{
The current approach also makes it hard to write composable dataplane
modules. To understand why, note that Even for a fixed target, 
 \hse{The dataplane pipeline model of an } }
Architecture usually contains packet-processing blocks with
heterogeneous execution models, also called \emph{abstract
machines}~\cite{p4lang,van1991machine}. For example\deleted{ 
consider PSA} in
\cref{fig:arch-example}, \hse{the parser blocks are} described using
...
flow~\cite{p4lang}. As \hse{behavior of each type of 
programmable-block} \deleted{abstract machine}is \hse{expressed using 
different sub-languages and constructs of P4, } \deleted{defined 
independently}
\hse{P4 lacks} uniform or compatible interface \deleted{that 
programmers can use}
\hse{to express composition of packet-processing modules through
programmable-blocks with different abstract machines.}
\deleted{write their packet-processing modules against.}
\deleted{This lack of uniform abstract
...
interfaces.}
Going back to the earlier example, in
...
programming model that hinders code reuse.\deleted{portability, 
modularity and composition:}
\begin{enumerate}
  \item \hse{P4 programs are architecture-specific.}
    Different architectures vary in their dataplane pipeline model.
    \deleted{which have incompatible structure and semantics.} 
    This renders P4 programs written for one architecture not 
    amenable to be ported to others.
  \item Architectures do not provide \deleted{a clean abstraction 
that can
    decouple dataplane programs from targets and also do not enforce}
    sufficient \hse{encapsulation and} scoping of information 
\hse{across programmable blocks}. This makes it difficult to
...
\hse{In summary, P4 programs are $(1)$ \emph{non-portable} because
of their affinity to architectures, $(2)$ \emph{monolithic} 
because of huge data-sharing across processing-blocks within the 
pipeline and $(3)$ \emph{non-composable} because of heterogeneity 
in abstract machines of processing-blocks and the lack of uniform 
interface.}
...
In summary, there are two main challenges for easy reuse of code with existing data plane programming ecosystem.  $(1)$ A lack of higher-level of abstraction for data plane pipeline to decoule P4 code from target devices and $(2)$ Absence of homogeneous abstract-machines or mechanism for seamless transition across heterogeneous abstract-machines involved in various packet-processing phases.
}
...
\hse{Our main design goal is to make P4 programs portable, modular, 
and composable.}
\deleted{
...
supports modularity and composition. These programs are written 
with the \uarch architecture. While compiling, users specify the real
target architecture \hse{for} which the \ulang programs should be
\hse{translated}. This decouples programs from target architectures 
while keeping the workflow very simple and intuitive.
...
within each module. A module can be thought of receiving a partial or
\hse{complete}
...
\hse{express special processing (e.g., packet replication)}
\deleted{use target-specific externs} by introducing logical externs 
that each modules may use.

\ucomp \hse{transforms} \ulang code \hse{for} a specified target 
architecture while
...
across modules, enabling \hse{their composition} and 
facilitating code reuse. 
\hse{In the second step, it translates all the control blocks 
and \uarch-specific constructs into the control blocks and 
constructs exposed by the target architecture's dataplane pipeline 
model.}
\deleted{In the second step, it translates the homogeneous control 
blocks into target-specific control blocks based on the pipeline model 
of the specified target architecture while translating \uarch-specific
...
Now, we describe each component of \ulang to which enable our goals
for dataplane programming.

\paragraph{\uswitch.}
\uswitch is an intermediate logical target with an abstract
programming model. It is designed to provide a high-level
abstraction \hse{that enables modular programming for dataplane.}
\deleted{packet processing.}
\deleted{the dataplane pipeline to enable code reuse agnostic
of architectures of target devices, and do so without compromising on
expressiveness for processing packets.} 
It enables modular programming
by allowing to write modules that implement fine-grained
packet-processing and defining interfaces to \hse{compose them}.
\begin{figure}[htb]
    \centering
    \includegraphics[width=\linewidth]{uswitch-model}
    \caption{Programming Model for \uswitch}
    \label{fig:uswitch}
\end{figure}

\uswitch models each packet-processing module as 
a self-contained,\hse{black-box} execution unit, called a 
\emph{\uprogram}, and executes it using the model shown in 
\cref{fig:uswitch}. A \uprogram takes as input: a packet byte-stream, 
intrinsic metadata and arguments for user-defined
parameters. As output, it generates byte-streams, metadata, and 
return values corresponding to one or more packets. Such a model
hides implementation details, such as headers types, user-defined 
metadata and programmable blocks, of a \uprogram from other 
\uprograms. \uswitch fetches an element from a logical input buffer 
for each \uprogram, executes the \uprogram and writes one or more 
elements to the logical output buffer, which acts as input for 
another. Note that these buffers are \hse{just} an abstraction, and 
they \hse{do not represent} buffers in \hse{real 
target devices.}

\paragraph{\uswitch Architecture (\uarch).}
To write \hse{portable \uprograms}, we provide \uswitch Architecture
(\uarch) that defines the dataplane as composition of logical 
pipelines, called \emph{\upipeline}, where each \upipeline implements 
a part of the complete packet processing on the device. Each 
\upipeline is associated with an interface declared in the 
architecture. Every interface specifies generic run-time parameters 
and declares programmable blocks required to be implemented for a 
pipeline. In order to raise level of abstraction for packet processing 
in dataplane pipeline, \upipelines comprise of \emph{only programmable 
packet-processing blocks}, parser, match-action control blocks and 
deparser. In addition, \uarch declares  intrinsic metadata and 
\emph{logical} externs to facilitate all the packet-processing 
features, e.g., multicast, cloning, that are not part of P4 language 
but supported by architectures of real-targets. Programmers can use 
the logical externs in to use such packet-processing features. 
\deleted{Using  constructs declared in \uarch, programmers can write 
modules containing one or more \uprograms.}
...
implementing interfaces declared in \uarch. The interfaces enable 
\upackages to have run-time behavior, enabling composition \upackages.
}
\hs{TODO: cont}
...
\deleted{available in fixed-function blocks (e.g., packet duplication) of pipelines models of real-target architectures.}
...
\hs{TODO: behavior of apply method call of each interface}
...
% \texttt{in\_buf} and \texttt{out\_buf} externs represent logical input and output buffer of \uswitch packet-processing model (Figure \ref{fig:mp4-packet-processing-model}).
...
\subcaption[]{ModularRouter \uprogram with main instance}\label{subfig:router-main}
\end{minipage}
\caption[]{Modular Router using \uarch \footnotemark}
\label{fig:modular-router}
\end{figure*}
\footnotetext{For brevity, we have skipped the unused parameters in parser, control blocks and package type declarations of \uprograms.}
...
The \texttt{ModularRouter} program in Figure \ref{subfig:router-main}, parses the ethernet header and invokes the instance \texttt{route} of \texttt{router}, shown with highlighted code.
...
\hs{TODO: this Section \ref{subsubsection:packet-processing-schedule} will move out of paper of in appendix. In that scenario, modify  above para to hint convincing solution to reader}
...
% All other common program statements not accessing \texttt{pkt} instances are also excluded from threads.
...
% In presence of conditional statements, a thread may process a packet copied from one of multiple possible packet instances.
...
The features of \ulang and constructs defined in \uarch enable novel ways of expressing data plane programs of network applications.
...
Therefore, developing modules responsible for processing a layer or set of layers is a natural way to design data plane programs, which can be easily reused and developed incrementally.\\
\textbf{Layered Modularity.}
...
We have already shown Modular Router (Figure \ref{fig:modular-router}) decoupling TCP/IP layer-2 and layer-3 processing.\\
...
% \end{figure}
\textbf{Incremental Development.} In the \texttt{router} program, we incrementally develop novel routing mechanism, SRv6, by adding independently developed a \uprogram (\texttt{srv6}) to process IPv6 Segment Routing Header (SRH) \cite{srh}.
\begin{lstlisting}[frame=none, escapechar=!]
0x86DD : {
...
\end{lstlisting}
Above code fragment shows a portion of code from modular router (Figure \ref{subfig:router-main}) to integrate \texttt{srv6} program.
We add support for \texttt{srv6} without modifying any other \uprogram.


\subsection{Composing Data Plane Programs of NFs}
\label{subsection:composing-dataplane-programs-of-NFs}

Many network functions (e.g., Firewall, NAT) may process packets across protocol layers.
Network administrators need to deploy data plane programs of such applications on a subset of devices and/or using different combinations and configurations of the programs.
Previous work on data plane composition (P4Bricks \cite{soni:hal-01632431} and P4Visor \cite{Zheng:2018:PLV:3281411.3281436}) and virtualization (HyPer4 \cite{Hancock:2016:HUP:2999572.2999607} and HyperV \cite{8038396}) define operators based on selective use cases.
\uswitch facilitates natural composition of such operators using new features (user-defined package types) of \ulang and native ones of P4, (e.g., conditional statements in, imperative control).\\
\textbf{Composition Operators.} Following snippet of code from implementation of orchestration interface shows use of \texttt{if} statement to express \emph{sequential} and \emph{override} (CoVisor \cite{188954}) composition.
\begin{lstlisting}[frame=none, escapechar=!]
...
\end{lstlisting}
The \texttt{firewall} may drop packets by analysing headers of Ethernet, IP, TCP and UDP or more protocols.
Packets are routed only if they are not marked for drop.
Similarly, \texttt{elephant\_flow\_routing} overrides normal routing.
Finally, \texttt{p4\_int} appends In-band Network Telemetry (INT) \cite{Kim2015InbandNT, p4int} headers with basic ingress and egress information.
Though \uarch does not have notion of ingress and ingress pipelines, but it declares intrinsic metadata which can be mapped to INT data.
For example, \emph{ingress timestamp} of INT is mapped to \texttt{IN\_TIMESTAMP}.
\ucomp partitions the composed pipeline and allocates \texttt{p4\_int} to egress control block of v1model architecture.

Another example code fragment shown below implements of \texttt{A-B Testing} composition operator defined by P4Visor\cite{Zheng:2018:PLV:3281411.3281436}.
\begin{lstlisting}[frame=none, escapechar=!]

...
\cref{fig:ether.p4,fig:ipv4.p4}. The first code fragment, \texttt{ether},
parses the Ethernet header, and modifies the source and destination
addresses using the next hop (\texttt{nh\_id}), which is supplied as
an argument, and finally deparses the packet. The second code
...
and, finally, deparses the packet. Note that neither \texttt{ipv4} nor
\texttt{ether} is a complete packet-processing program: the former does
not generate a functionally correct packets, while the latter is
parameterized on the next hop and so does not specify forwarding
behavior. However, we could use \texttt{ether} with any other routing
scheme (e.g., IPv6, MPLS, etc.) to obtain a valid program.

\begin{figure}[ht]
\begin{lstlisting}[frame=none, escapechar=!]
// ether.p4: parse and process only Ethernet header
...
\end{enumerate*}
Users write individual modules for specific processing---for
example, \texttt{ether.\ulang} in \cref{fig:compile-module} performs only
ethernet processing similar to \cref{fig:ether.p4}.
\cref{fig:compile-module} shows the first stage of compiling where
\ucomp compiles a module into \uarch-specific IR.  During this, \ucomp
transforms parser and deparser blocks into match-action control. This
transformation unifies the abstract-machines for packet-processing,
thus allowing seamless transfer of execution control within and across
modules and enables composition. In the second phase, users compose
the modules together to build a dataplane---e.g., in
\cref{fig:compile-to-target}, \texttt{main.\ulang} composes Ethernet and IPv4
modules to build a modular router. Further, based on the specified
target device, the compiler also generates the dataplane configuration
specific to the device through a sequence of intermediate steps
(\cref{sec:compiler}).
...
In this section, we explain \uarch's logical constructs enabling the 
programming model (\cref{fig:uswitch}) for the \uswitch. We designed 
the constructs that are amenable to the \ucomp translations for 
portability without compromising on expressiveness. In 
\cref{sec:pipelines}, we explain \upipelines and interfaces 
programmers require to implement in order to declare \uprogram types. 
\cref{sec:logical-externs} explains a set of logical externs, which 
can be used to express packet-processing functionality in \uprograms.


\subsection{\upipelines and Interfaces}
\label{sec:pipelines}
\uarch allows to program its dataplane using composition of two 
types of \upipelines, \emph{Simple} and \emph{Compound}. The simple 
type provides a basic model for processing, in which a single packet 
is processed by parser followed by match-action control and 
finally, deparser. From a single packet, a simple type of \upipeline 
may generate multiple packets that are individually processed 
in isolation of others by the same \upipeline, whereas the compound 
type of \upipelines can process multiple copies of a packet in the 
same context with different instances of \upipelines 
(\cref{fig:pipe-types}).

\uarch exposes three interfaces, \texttt{Unicast}, \texttt{Multicast} 
and \texttt{Orchestration}, to express composition of its pipelines. 
Every interface is associated with a type of \upipelines. To 
implement an interface programmers need to specify behaviour for all 
the processing-blocks of the associated type of \upipelines. By 
implementing an interface, programmer can declare a \uprogram having 
run-time behavior with an interface for composition. The distinct 
interface types provides explicit information about behavior of 
\uprograms to users. \hse{about 'clean' interface, can we 
use clean word?}

To comply with the model shown in \cref{fig:uswitch}, 
\uswitch implicitly performs few operations while invoking 
\uprograms. \uswitch fetches an element from logical input buffer 
for the \uprograms with \texttt{Unicast} or \texttt{Multicast} 
interface but for the ones with \texttt{Orchestration}, from the 
input buffer in their run-time parameters. \uswitch performs 
necessary queuing operation for the \uprograms with \texttt{Unicast} 
interface.
...
Unicast<H,M,I,O,IO>(pkt p, inout sm_t sm, es_t es, 
  in I i_param, out O o_param, inout IO io_param);
Multicast<H,M,I,O> (pkt p, in sm_t sm, es_t es, 
...
\tightparbf{Frontend:} generates \ucomp specific IR, \ucomp-IR. It 
...
\begin{figure}[tbh]
  ...
  \label{fig:workflow}
\end{figure}
\tightparbf{Midend} composes \uprograms by homogenizing their 
processing-blocks. For that, it performs static analysis and 
transforms the parsers-deparsers into match-action control blocks. 
Also, in case of compile-time replication, it extracts 
packet-processing code for every replica and prepares processing 
schedule. The Midend passes are independent to target architectures, 
focusing on composition. 
\tightparbf{Backend} passes are specific to a target architecture. 
They transform the \ucomp-IR, comprising only control blocks with 
\uarch-specific constructs, to the target architecture-specific IR.
While mapping the control blocks to the ones declared by pipeline 
model of the target architecture, it translates \uarch-specific 
logical constructs to the target specific ones.
...
\tightparbf{1. Packet Header Vector (PHV)} stores parsed headers,
...
\tightparbf{2. Number of Match-Action Hardware Stages.}%

...
Programmers provide P4 code for the programmable blocks in the pipeline model described in the architecture of the target device.
...
% the natural reuse of packet-processing code.
...
\section{MicroP4}
$\mu$P4 is an intermediate logical target with an abstract packet-processing model.
...
We design a Micro-Switch Architecture ($\mu$ SA) that defines data plane model with multiple logical pipelines to develop $\mu$s-programs.
...
$\mu$ SA data plane pipelines comprise of only programmable packet-processing blocks, parser, match-action control blocks and deparser. 
In addition, $\mu$ SA declares intrinsic metadata and logical externs to facilitate all the packet-processing features, e.g., multicast, cloning, that are not part of P4 language but supported by architectures of real-targets. 
Programmers can use the logical externs in $\mu$ SA pipelines to use such packet-processing features.
...
Using logical data plane model of $\mu$ SA, programmers can develop modular P4 code which is specific to $\mu$ SA.
We design and develop MicroP4 Compiler ($\mu$P4C) to compile and transform $\mu$s-program for architectures of real target devices, making $\mu$s-programs agnostic of architectures of real-targets.
...
$\mu$P4C extends current P4 language and allows programmers to define \texttt{package} types, declare their instances and invoke them using the built-in \texttt{apply} method.
...
$\mu$P4C.
$\mu$P4C homogenizes abstract-machines of programmable blocks by transforming parser and deparser blocks into match-action control blocks.
...
$\mu$P4C perform static analysis to estimate possible packet sizes required to process all the transformed $\mu$s-programs in match-action control blocks of target architecture's pipeline.
$\mu$P4C synthesizes P4 code for parser and deparer blocks of target architecture's pipeline to extract and emit required number of bytes from and to packet.
Next, $\mu$P4C translates intrinsic metadata and logical externs into the target architecture-specific constructs.
...
Using the packet-processing model described in Figure \ref{fig:mp4-packet-processing-model}, we define $\mu$s-program interfaces and packet-processing pipelines for $\mu$P4 as a part of its data plane architecture, $\mu$ SA.
In addition, $\mu$ SA defines a set of logical externs providing various packet-processing constructs available in fixed-function blocks (e.g., packet duplication) of pipelines models of real-target architectures.
...
Orchestration<I,O>
  ...
  {Con<I,O> c};
\end{lstlisting}
$\mu$ SA declares threes interface types, \texttt{Unicast}, \texttt{Multicast} and \texttt{Orchestration}.
...
$\mu$ SA models packets using an extern object, \texttt{pkt}, declared in $\mu$ SA (Figure \ref{fig:micro-switch-architecture}.
The \texttt{pkt} extern can be instantiated inside control blocks of only orchestration pipelines.
$\mu$P4C mandates to initialize \texttt{pkt} instances in the control block using its \texttt{copy\_from} method.
...
$\mu$P4 considers this method as a compile-time known packet replication, whereas multicast as a run-time packet-replication described in Section \ref{subsubsection:multicast-extern}.
...
$\mu$SA defines intrinsic metadata for the logical target $\mu$P4.
For every real-target architecture , $\mu$P4C maintains mapping of its logical intrinsic metadata with the metadata of the architecture.
...
$\mu$ SA abstracts away correlated intrinsic metadata using stateful logical externs.
...
\end{minipage}
\caption[]{Modular Router using $\mu$ SA \footnotemark}
...
Figure \ref{fig:modular-router} shows a possible way to write modular data plane program for a router using interfaces and externs declared in $\mu$ SA.
...
$\mu$P4 allows programmers to configure packet replication at run-time using a multicast extern , \texttt{mc\_engine}, declared in $\mu$ SA.
...
$\mu$ SA declares three types of extern objects, \texttt{in\_buf}, \texttt{out\_buf}, \texttt{mc\_buf}.
...
The \texttt{in\_buf} extern has \texttt{dequeue} method that is used only by the logical device $\mu$P4. 
...
It is designed to  be used along with \texttt{mc\_engine} to express multicast packet-processing, where as \texttt{in\_buf} and \texttt{out\_buf} are facilitates packet-processing model of $\mu$P4.
...
$\mu$P4C transforms parsers and deparsers of the main and callee package types into match-action control blocks.
...
$\mu$P4C executes transformed parsers and deparsers in the match-action stages using the byte-stack.
...
$\mu$P4C discards types and instances of the headers of each package.
...
As $\mu$P4 allows nested calls of package instances, evaluation of extract-length and byte-stack size is a recursive process.
$\mu$P4C performs symbolic execution to evaluates extract-length and byte-stack size of every callee package in every control path in the CFG of the caller's control block.
...
$\mu$P4C parser should extract 78 bytes to process $(1)$ removal of mpls header by the first callee and $(2)$ the logest path (eth-IPv6-IPv4) in the parse-graph of the subsequent callee package.
\begin{figure}[!h]
    \centering
...
$\mu$P4C transforms the packages described using $\mu$P4's data plane abstractions into a given real target specific ones.

...
\uarch's logical externs provide a way to represent packets and
metadata as well as express packet-processing functions in a
target-agnostic manner. These generic constructs are mapped to
target-specific constructs eventually.

\tightparbf{Packet Extern.}
To represent a packet as a buffer element in \uswitch's model
(\cref{fig:uswitch}), \uarch provides an extern called \texttt{pkt}
(\cref{fig:micro-switch-architecture}). \uarch's \texttt{extractor}
and \texttt{emitter} externs allow operations to modify a \texttt{pkt}
instance. Users can also instantiate and initialize new \texttt{pkt}
instances using \texttt{copy\_from} method. While a \texttt{pkt} needs
to be initialized, \texttt{emitter} and \texttt{emitter} do not need
to be initialized.
...
\tightparbf{Correlated Intrinsic Metadata.}
...
in. These metrics are used to compute operational-region for 
the main \uprogram (discussed in 
\cref{subsubsection:static-analysis}).

Having a stack of bytes storing packets' operational-region allows 
to synthesise code that performs parse and deparse packets in control 
blocks. The next midend pass performs these transformations 
to homogenize all processing blocks to control blocks 
(\cref{subsection:parser-to-match-action-table}).
Naturally, the main \uprogram is also transformed to a control 
...
From the design of programmable parser \cite{6665172}, we 
...
\cref{subfig:parser-mat}) comprising all the assignment statements of 
the path states. A match-action entry is created by pairing 
appropriate key-set values and action for every path.
...
\texttt{emitter} instance's \texttt{emit} method-call statements with 
match-action table to transform deparser into normal match-action 
control blocks.

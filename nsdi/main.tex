...
Although \ucomp transforms and realizes many higher-level abstraction of data plane model defined in \uarch, there remians few sub-computations of packet-processing and functions of dataplane requiring abstraction to enhance portability.
...
For example, converting an entry with range kind of key into a set of entries with enumerating the range requires runtime translation from control plane.
\ucomp addresses on only compile-time transformations
Similarly, \uarch and \ucomp lacks abstractions and corresponding transformation to concrete implementation for checksum computations, action selector, action profiles.

\textbf{Communication to control plane.}
sending packet to control plane from data plane,  digest, to cpu


\textbf{Stateful Memories.} 
No need of so many types of stateful Memories. counters, meters, registers.
analyse programs and allocate metadata based on scope to counters, meter or registers.

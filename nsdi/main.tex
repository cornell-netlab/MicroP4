...
architectures and targets which hurts portability and also prohibits
modular development. For example, many P4 programs written for PSA
rely heavily on using intrinsic metadata supported by PSA, and such
metadata is used across a program in an unrestricted
manner~\cite{switch.p4}; it is difficult to split such a program into
clean modules which can be developed independently.  More
...
\todo{``pipeline'', normally, refers to a set of stages that process packet
end-to-end. But, in this context, we are using pipeline to refer to
one small portion within an actual pipeline. Is there a better term to
use other than pipeline? Maybe, $\mu$-pipeline?}

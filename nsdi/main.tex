...
control Pipe(pkt p, inout hdr_t h, inout sm_t sm, es_t es, inout bit<16> meta) {
  ...
  apply {
    l3_inst.apply(p, sm, es, nh_id, meta);
    forward_tbl.apply(); 
...
As a part of data plane abstractions, $\mu$SA allows to describe single and multiple packet processing in control blocks of its orchestration pipelines.
...
$\mu$P4C's transformations create copies of packet using externs provided by the architectures of the specified target.
$\mu$P4C realizes multi-packet processing by identifying a sub-CFG, called a \emph{thread}, for every copy of a packet.
% Existing target devices can not process multiple packets by the same thread at the same time.
Then, it prepares Packet-Processing Schedule(PPS) of the threads in the program and maps them to packet-processing blocks of the target's architecture.
...
We derive a Program Dependence Graph(PDG) \cite{Ferrante:1987:PDG:24039.24041} from the control block of every orchestration pipeline's interface.
$\mu$P4C performs a series of transformations on PDG to identify packet-processing threads for every instance of packet.
It first converts PDGs into Static Single Assignment(SSA) form to synthesize packet instances for every value computed in the program.

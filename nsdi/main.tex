...
\begin{lstlisting}[frame=none, escapechar=!]
0x86DD : {
  !\colorbox{mygray}{srv6\_i.apply(p, im);}! // copies next segment address from SRH header's list to IPv6's destAddr field
  ipv6_i.apply(p, im, h);
}
\end{lstlisting}
...
sflow(pkt, im_t);// Implements Unicast 
control con(pkt pk, im_t im, 
    out_buf<e_t> ob) { // `ob' holds out packets
  pkt pk2;  im_t im2;
  in_buf<e_t> ib; // in_buffer for another module
  apply {
    pk2.copy_from(pk); // copying
    im2.copy_from(im); // copying
    sflow.apply(pk2, im2); // sflow on copy
    ob.enqueue(pk, im); // puting original in ob
    ob.enqueue(pk2, im2); // copy in ob
...
to PSA. PSA exposes a dataplane model that target device must 
realise. Instead, we focus on abstraction that programmers should use 
for portability.

Our work is more inline with the Click Modular Router 
\cite{Kohler:2000:CMR:354871.354874}.

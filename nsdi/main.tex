...
\begin{document}
\date{}
\title{\Large \bf \ulang: Towards [(Compositional) (Modular) (Reusable)] Dataplane [Programm(s)(ing)]}
...
We present the design of the \uswitch logical architecture (\uarch),
...
Using the packet-processing model described in Figure \ref{fig:mp4-packet-processing-model}, we define \uprogram interfaces and packet-processing pipelines for \uswitch as a part of its data plane architecture, \uarch.
...
\uswitch considers this method as a compile-time known packet replication, whereas multicast as a run-time packet-replication described in Section \ref{sec:multicast}.
...
}!{\subcaption[]{ipv6 \uprogram}\label{subfig:ipv6}}!
/* router.p4 */
// Declarations of !{\color{commentcolor}{\uprograms}}!
...
\subcaption[]{Router's main \uprogram}\label{subfig:router-main}
\end{minipage}
...
\subsubsection*{Example \uprograms}
...
\end{lstlisting}
\caption{srv6 \uprogram to support IPv6 SRH}
\label{subfig:srv6}
\end{subfigure}
\begin{subfigure}[b]{\linewidth}
 \begin{lstlisting}[frame=none, escapechar=!]
...
\end{figure}
In the \texttt{Router} program, we can easily add support for yet another routing mechanism, SRv6, by independently developing a \uprogram (Figure \ref{subfig:srv6}) processing IPv6 Segment Routing Header (SRH).
...
We note that support for \texttt{srv6} is added without modifying \uprogram of any other routing scheme.

...
Now, we define extract-length ($lc_{a}(x)$) of control path $x$ in the CFG of program $a$'s control block as a function of extract-length ($\mathcal{E}l(cp)$) of and maximum decrease in packet size ($\delta(cp)$) by the callee programs in the path.

...
reconfigurable match-action tables. However, P4 programs are tightly
coupled to the underlying hardware architectures, which makes it
difficult to write composeable libraries of code that can be reused
...
abstracts away from the structure of the underlying hardware pipelines
used to implement them and naturally supports powerful forms of
composition. Using realistic examples, we show how \ulang enables a
modular style of program construction and we present an implementation
of the \ucomp compiler that generates code for the lower-level
architectures supported by current devices.
\end{abstract}
...
chips, while domain-specific languages such as
P4~\cite{bosshart2014p4, p4lang} enable specifying rich
packet-processing functions in terms of high-level abstractions such
as parsers and match-action tables.
...
accommodate a wide range of targets, it also creates a tight coupling
...
in \cref{fig:ether.p4,fig:ipv4.p4}. The first fragment,
\texttt{ether}, parses the Ethernet header, modifies the source and
destination addresses using the next hop (\texttt{h}) which is
supplied as an argument, and finally deparses the packet. The second
fragment, \texttt{ipv4}, parses the IPv4 header, uses longest-prefix
matching to determine the next hop, decrements the \texttt{ttl} field,
and deparses the packet. Note that neither \texttt{ether} nor
\texttt{ipv4} is a complete packet-processing program: the latter is
parameterized on the next hop and so does not specify forwarding
behavior, while the latter does not generate a valid packet (at least,
not one that is well-formed according to the standard networking
stacks implemented by end hosts). However, if we can combine the code
in \texttt{ether} with any routing scheme (e.g., IPv6, MPLS, etc.), we
can obtain a complete program.

Unfortunately combining P4 program fragments is not easy as programs
are written against an architecture whose structure is based on the
underlying hardware pipeline. For example,
\texttt{switch.p4}~\cite{switch.p4} handles several dozen different
protocols and functions---e.g., Ethernet switching, IPv4 and IPv6
routing, tunneling, access control, etc. However, the code is written
against the V1Model architecture (see ~\cref{fig:arch-example}) in a
monolithic style, and uses a fixed set of headers, user-defined
metadata, and architecture-defined standard metadata. To use
...
functionality from the rest of the program. Without a detailed
understanding of the overall structure of the top-level program, doing
this correctly would be extremely difficult. In practice, programmers
rely on C preprocessor directives to enable and disable various
features---an ad hoc and fragile approach.

This paper presents the design and implementation of \ulang, a new
architecture that provides fine-grained abstractions for constructing
dataplane programs. \ulang provides a new architecture that distills
packet processing to its essence and abstracts away hardware-level
structure. \ulang enables writing P4 programs in a modular style, by
combining functionality from simple programs and reusable libraries of
code to obtain richer forms of packet processing. To implement these
features, the \ucomp compiler maps composite programs onto standard P4
architectures, merging parsers and match-action tables from multiple
programs and scheduling them onto a single packet-processing pipeline.

There is some prior work enabling modular composition in the context
of P4. Systems such as HyPer4 \cite{Hancock:2016:HUP:2999572.2999607},
HyperV~\cite{8038396}, and
P4Visor~\cite{Zheng:2018:PLV:3281411.3281436} focus on merging
multiple independent programs so they can execute a single device.
However, unlike \ulang, these systems only handle complete programs
and lack mechanisms for selectively reusing code fragments from
libraries, for specifying interfaces between modules, and for
describing inter-module communication.
...
transforms \ulang programs \deleted{, which are \uarch-specific,} for 
a specified target architecture. We focus on key transformations that 
enable composition and help \uarch to realize portable and 
modular programming.

\begin{figure*}[tbh]
  \centering
  \includegraphics[width=\linewidth]{workflow}
  \caption{Workflow for building dataplanes in \ulang. \todo{update}}
  \label{fig:workflow}
\end{figure*}
...
hardware stages.
\deleted{match-action control blocks.}
...
\texttt{ipv4.protocol}---with their evaluated offsets in the 
byte-stack (\cref{subfig:parser-symbolic-execution}). Also, it 
performs Forward Substitution \cite{Padua:1986:ACO:7902.7904} on every 
path to eliminate anti-dependency between two states (\texttt{var\_y} 
replaced with \texttt{meta.data1} one \texttt{meta.data2} in another).
...
Using all the byte-stack offsets and variables used in \texttt{select} 
expressions of states in every 
path, (\cref{subfig:parser-symbolic-execution}), \ucomp
synthesizes a match-key(\cref{subfig:parser-mat}). 
For each path, to assert that packet is long enough to parse, the 
match-key also comprises the valid bit flag of the last byte extracted 
on the path.
Then, for each path, we synthesize an
...
fourth keys are matched with don't care values. Finally, the midend 
transforms header fields used in the control block of every
\uprogram to necessary metadata called, \emph{field-variables}, that
are copied from byte-stack in the synthesised actions. The 
field-variables include a valid flag for header instances. 
...
\Cref{tab:loc-of-composed} shows line of P4 code generated for 

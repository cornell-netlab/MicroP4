...
It declares intrinsic metadata and logical externs to facilitate all the packet-processing features, e.g., multicast, cloning, not part of P4 language but supported by architectures of real-targets. 
...
Each $\mu$s-program specializes a generic run-time interface declared in $\mu$ SA.
...
We design and develop MicroP4 Compiler ($\mu$P4C) to compile and transform $\mu$s-program for architectures of real target devices, making $\mu$s-program agnostic of architectures of real-targets.
...
$\mu$P4C homogenizes abstract-machines of programmable blocks (parser, match-action control blocks, deparser) declared for $\mu$ SA pipeline.
Programmers can use sub-languages of P4 to program parser and control blocks to describe $\mu$s-programs, $\mu$P4C transforms parser and deparser blocks into match-action control blocks.
...
\hse{TODO: mention P4 language extension: about user define packages, $\mu$s-program is a package type}

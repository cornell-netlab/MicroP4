...
translate constraints imposed by the target architecture on its 
intrinsic metadata.
It allocates the control blocks of \uarch-IR while respecting the 
constraints to the processing blocks exposed by the target 
architecture. During allocation, it also translates the intrinsic 
metadata.
...
\ucomp visits PPS nodes in topological order to map thread
...
prog(pkt, es_t, out h_t);
test(pkt, es_t, out h_t);
log(pkt, es_t, in h_t, in h_t);
control validate(pkt p, es_t e, out_buf<e_t> ob) {
...
/*  3  */   pt.copy_from(p); // c3 
/*  3  */   et.copy_from(e);
/* 2,1 */   prog.apply(p, e, hp); 
/* 3,1 */   test.apply(pt, et, ht); 
/*  1  */   if (hp != ht) {
/*  1  */     log.apply(pm, em, hp, ht);
/*  1  */     ob.enqueue(pm, em);
/*  1  */   }
/*  3  */   et.set_out_port(DROP);
/*  2  */   ob.enqueue(p, e);
/*  3  */   ob.enqueue(pt, et);
          }
       }

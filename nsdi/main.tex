...
supports modularity and composition. These programs, \hse{called 
\uprograms}, are written with
...
\ucomp \hse{translates} \ulang code to a specified target 
architecture while
...
packet-processing model. It is designed to provide a high-level
abstraction for the dataplane pipeline to enable code reuse agnostic
of architectures of target devices, and do so without compromising on
expressiveness for processing packets. It enables modular programming
by allowing to write modules that implement fine-grained
packet-processing and defining interfaces to hide implementation.
\begin{figure}[htb]
    \centering
...
packet byte-stream, intrinsic metadata and arguments for user-defined
...
implementation details, such as headers types, user-defined metadata
and programmable blocks, of a \uprogram from other \uprograms.
\uswitch fetches an element from a logical input buffer for each
\uprogram, executes the \uprogram and writes one or more elements to
the logical output buffer, which acts as input for another. Note that
these buffers serve to provide an abstraction, and they do not
necessarily mean that we need to allocate buffers in the hardware when
mapping to real architectures.

\paragraph{\uswitch Architecture (\uarch).}
\deleted{\todo{``pipeline'', normally, refers to a set of stages that
process packet end-to-end. But, in this context, we are using pipeline
to refer to one small portion within an actual pipeline. Is there a
better term to use other than pipeline? Maybe, $\mu$-pipeline?}}

To write programs for \uswitch, we provide \uswitch Architecture
(\uarch) that defines the dataplane as consisting of two types of
\emph{\uarch pipelines}, where each $\mu$-pipeline implements a part
of the complete packet processing.

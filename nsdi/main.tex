...
Orchestration<I,O>
  ...
  (Con<I,O> c);
\end{lstlisting}
$\mu$ SA declares threes interface types, Unicast, Multicast and Orchestration.
...
!\colorbox{mygray}{ipv6}!(pkt p, inout sm_t sm, es_t es, out bit<16> nexthop);
!\colorbox{mygray}{ipv4}!(pkt p, inout sm_t sm, es_t es, 
    out bit<16> nexthop);
!\colorbox{mygray}{mpls}!(pkt p, inout sm_t sm, es_t es);
\end{lstlisting}
...
control C(pkt p, inout hdr_t h, inout sm_t sm, es_t es) {
  bit<16> nh_id;
  !\colorbox{mygray}{ipv4() ipv4\_i; ipv6() ipv6\_i; mpls() mpls\_i;}!
  action drop () {}           

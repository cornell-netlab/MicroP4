...
\begin{figure}[tbh]
  ...
  \label{fig:workflow}
\end{figure}
\tightparbf{Midend} performs two major transformations. \par
\textbf{First}, to compose \uprograms by homogenizing their 
...
parsers-deparsers into match-action control blocks.

\textbf{Second,} to preprocess for the backends. In case of 
compile-time replication, extracts packet-processing code for every 
...
\tightparbf{Backend} transformation are for two goals.\par
\textbf{First,} to allocate control blocks to the target's pipeline 
model while translating to and respecting constraints on its 
intrinsic metadata. \\ \textbf{Second,} to synthesizes required 
parser and deparser blocks in target's pipeline model. 
The backend is a specific to a target  architecture.
...
key = { !{\color{parser-red}{b[12]++b[13]}!:exact; // rest Ternary
  !{\color{parser-red}{b[20]; b[23]; meta.data1; meta.data2;}!
  b[53].isValid(); b[73].isValid();} // length
...
(!{\color{parser-green}{x86DD,x6}!,_,!{\color{parser-green}{xFF}!,_,_,1):cp_eth_ipv6_tcp();
}
...
\tightparbf{Parser.} From the design of programmable parser 
\cite{6665172}, we 
...
\tightparbf{Deparser.}Using the emit order of header instances and 
...
\texttt{im\_t} object.
% We perform trivial translation of method calls of \uarch's extern 
...
compiler, \texttt{p4c}~\cite{p4c}. \deleted{For any P4-specific 
components, our implementation conforms to the P4-16 
specification~\cite{p4lang}.} In
...
backend. We have made our implementation available under an
open-source
...
We briefly highlight subtle differences in \ulang compared to P4-16.
...
target-specific translations.} As described in 
\cref{sec:compiler}, \ucomp's only backend
(\cref{subsection:micro-backend}) is
target-specific as it translates the target-independent \uarch IR into
a P4 program for the specified target. Because it maps
intrinsic metadata of \uarch to that of the target architecture, it 
needs target architecture-specific mappings for metadata and
methods corresponding to \texttt{meta\_t}, \texttt{im\_t}, 
\texttt{es\_t} and \texttt{mc\_engine}. These mappings are used for 
translation. The backend transforms the IR to conform pipeline model
...
74$. In this scenario, the caller's byte-stack header would have 
lower global utilization of PHV compared to \texttt{mp}'s 
headers. Of course, this naive analysis considers overheads at the 
source-level. A smart
...
increase the local utilization of PHV in the hardware stages.
...
RMT-based devices have limited number of pipeline stages to execute

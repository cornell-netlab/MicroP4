...
\tikzset{
    queuei/.pic={
  \stepcounter{cntr}
        \node[outer sep=0pt,draw, color=lightgray, rectangle split,rectangle split horizontal,minimum height=10em, rectangle split parts=2] (queue-\thecntr) [pic actions] {};
        \draw [color=lightgray]
          (queue-\thecntr.north west) -- ++(-0.5cm,0)
          (queue-\thecntr.south west) -- ++(-0.5cm,0);
        \node[above, fill=white, opacity=0.5] at ([xshift=-0.2cm]queue-\thecntr.south) {#1 buffer};
    },
}
...
% the shapes
\path
(-0.5,-1.2) pic[rectangle split part fill={white,white,gray!80},anchor=east] {queuei=}
(8,-1.2) pic[anchor=west] {queuei=};
...
\node (upipetext) at (upipe.south) [minimum height=0.5cm, text centered, text width=8em,anchor=north] {\large $\mu$-pipe control};
...
\node (opipetext) at (opipe.south) [minimum height=0.5cm, text centered, text width=16em,anchor=north] {\large orchestration-pipe control};

\path [line, -, dotted] (0,-1.2) -- (7,-1.2);

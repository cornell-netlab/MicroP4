...
\begin{figure}[h]
    ...
    \caption{Runtime Behavior of Package in $\mu$P4}
...
$\mu$P4C well-known compilation phases, compile and link, to generate executable programs.
If a micro-switch program does not contain main instance, $\mu$P4C compiles it to an Intermediate Representation (IR) in json format (\ref{subfig:compiling-modules}) to generate a library.
...
\end{figure}
Programmers can include files (*p4h) defining types (e.g., struct types of in and out parameters) used in runtime interfaces of packages in libraries.
